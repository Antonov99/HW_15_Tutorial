...
namespace PolyAndCode.UI
{
    ...
    /// </summary>
    public class HorizontalScroller<T> : IScroller where T : Component
    {
        ...
        public IDataAdapter<T> DataAdapter;
        protected T PrototypeCell;
        ...
        #region INIT
        public HorizontalScroller(RectTransform viewport, RectTransform content, IDataAdapter<T> dataAdapter, bool isGrid, int rows)
        {
            PrototypeCell = dataAdapter.ViewPrefab;
            Viewport = viewport;
            Content = content;
        ...
        /// <summary>
        /// Creates cell Pool for recycling, Caches Ts
        /// </summary>
        private void CreateCellPool()
        {
            ...
            {
                _cachedCells = new List<T>();
                _cellPool = new List<RectTransform>();
            }

            //Set the prototype cell active and set cell anchor as top 
            PrototypeCell.gameObject.SetActive(true);
            var prototypeTransform = PrototypeCell.GetComponent<RectTransform>();
            SetLeftAnchor(prototypeTransform);

            //set new cell size according to its aspect ratio
            _cellHeight = Content.rect.height / _rows;
            _cellWidth = prototypeTransform.sizeDelta.x / prototypeTransform.sizeDelta.y * _cellHeight;
            ...
            int minPoolSize = Math.Min(MinPoolSize, DataAdapter.DataCount);
            ...
            while ((poolSize < minPoolSize || currentPoolCoverage < requriedCoverage) && poolSize < DataAdapter.DataCount)
            {
                ...
                _cachedCells.Add(item.GetComponent<T>());
                DataAdapter.OnAttachItem(_cachedCells[^1], poolSize);
        ...
        /// <returns></returns>
        public Vector2 DoScroll(Vector2 direction)
        {
        ...
        private Vector2 RecycleLeftToRight()
        {
            ...
            while (_cellPool[leftMostCellIndex].MaxX() < _recyclableViewBounds.min.x && currentItemCount < DataAdapter.DataCount)
            {
                if (IsGrid)
                {
                    if (++_RightMostCellRow >= _rows)
                ...
                //Cell for row at
                DataAdapter.OnAttachItem(_cachedCells[leftMostCellIndex], currentItemCount);
        ...
        private Vector2 RecycleRightToleft()
        {
            ...
            while (_cellPool[rightMostCellIndex].MinX() > _recyclableViewBounds.max.x && currentItemCount > _cellPool.Count)
            {
                ...
                currentItemCount--;
                //Cell for row at
                DataAdapter.OnAttachItem(_cachedCells[rightMostCellIndex], currentItemCount - _cellPool.Count);
